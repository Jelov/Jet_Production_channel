...
//	t1->Add("../HiForestAOD_pp_MC_bjets.root");
//  t1->Add("../HiForestAOD_pp_MC_bjets_after41.root");
	t1->Add("/mnt/hadoop/store/user/chengchi/Pythia8_bjet30_5020GeV_GEN-SIM/MC_bJet30_HiForest/160215_053738/0000/HiForestAOD_pp_MC_bjets_after41*.root");
...
*/
  ...
  Canvas_pt1->Divide(3,2);
  Canvas_pt1->cd(1);
  t1->Draw("jtpt>>flavor_excitation_pt",flavor_excitation_cut&&eta_2less_cut);
  flavor_excitation_pt->GetXaxis()->SetTitle("jet_pt");
  flavor_excitation_pt->Draw();
  Canvas_pt1->cd(2);
  t1->Draw("jtpt>>pair_creation_pt",pair_creation_cut&&eta_2less_cut);
  pair_creation_pt->GetXaxis()->SetTitle("jet_pt");
  pair_creation_pt->Draw();
  Canvas_pt1->cd(3);
  t1->Draw("jtpt>>gluon_splitting_pt",gluon_splitting_cut&&eta_2less_cut);
  gluon_splitting_pt->GetXaxis()->SetTitle("jet_pt");
  gluon_splitting_pt->Draw();
  Canvas_pt1->cd(4);
  t1->Draw("jtpt>>isr_pt",isr_cut);
  ...
  Canvas_pt1->SaveAs("Jetpt_eta_2less.pdf");

  TCanvas *Canvas_pt2 = new TCanvas("jet_pt_eta2more","jet_pt_eta2more");
  Canvas_pt2->Divide(3,2);
  Canvas_pt2->cd(1);
  t1->Draw("jtpt>>flavor_excitation_pt",flavor_excitation_cut&&eta_2more_cut);
  flavor_excitation_pt->GetXaxis()->SetTitle("jet_pt");
  flavor_excitation_pt->Draw();
  Canvas_pt2->cd(2);
  t1->Draw("jtpt>>pair_creation_pt",pair_creation_cut&&eta_2more_cut);
  pair_creation_pt->GetXaxis()->SetTitle("jet_pt");
  pair_creation_pt->Draw();
  Canvas_pt2->cd(3);
  t1->Draw("jtpt>>gluon_splitting_pt",gluon_splitting_cut&&eta_2more_cut);
  gluon_splitting_pt->GetXaxis()->SetTitle("jet_pt");
  gluon_splitting_pt->Draw();
  Canvas_pt2->cd(4);
  t1->Draw("jtpt>>isr_pt",isr_cut);
  ...
  Canvas_pt2->SaveAs("Jetpt_eta2more.pdf");
